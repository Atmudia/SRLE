...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		void GetTarget()
		{
			if(nearAxis == Axis.None && Input.GetMouseButtonDown(0))
			{
				...
				if(Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, selectionMask))
				{
					...
					bool activate = false;
					List<ObjectAddedBySRLE> objectAddedBySrles = new List<ObjectAddedBySRLE>();
					objectAddedBySrles.AddRange(target.GetComponentsInChildren<ObjectAddedBySRLE>());
					objectAddedBySrles.AddRange(target.GetComponents<ObjectAddedBySRLE>());
					objectAddedBySrles.AddRange(target.GetComponentsInParent<ObjectAddedBySRLE>());

					foreach (var objectAddedBySrle in objectAddedBySrles)
					{
						target = objectAddedBySrle.gameObject.transform;
						activate = true;
					}
		...
		void SetMaterial()
		{
			...
			{
				lineMaterial = new Material(EntryPoint.shaders["Custom/Lines"]);
				outlineMaterial = EntryPoint.srlegizmo.LoadAsset<GameObject>("WireFrame").GetComponent<MeshRenderer>()
					.material; //new Material(EntryPoint.shaders["VR/SpatialMapping/Wireframe"]);
				//outlineMaterial = new Material(EntryPoint.shaders["Custom/Outline"]);
			}
		}
