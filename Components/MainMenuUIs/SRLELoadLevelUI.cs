...
namespace SRLE.Components
{
	...
	{
		public List<LevelData> availLevels = new List<LevelData>();
		...
		public IEnumerator FinishUpdateAvailGames()
		{
			...
			foreach (KeyValuePair<string, LevelData> keyValuePair in Directory.GetFiles(SaveManager.LevelsPath, "*").ToDictionary(x => x,
				         x =>
				         {
					         var levelData = Newtonsoft.Json.JsonConvert.DeserializeObject<LevelData>(File.ReadAllText(x));
					         levelData.Path = x;
					         return levelData;
				         }))
			...
			foreach (LevelData availGame in loadGameUi.availLevels)
			{
		...
		public GameObject CreateLoadGameButton(LevelData gameSummary)
		{
		...
		public void SetSelectedIdx(int idx)
		{
			...
			try
			{
				...
				this.summaryPanel.gameObject.FindChild("DeleteButton", true).GetComponent<Button>().onClick.AddListener(DeleteSelectedLevel);
