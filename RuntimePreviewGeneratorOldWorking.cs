...
public static class RuntimePreviewGenerator
{
...
#endif
	{
		...
		try
		{
			...
			try
			{
				...
				if( asyncCallback != null )
				{
					AsyncGPUReadback.Request( renderTexture, 0, m_backgroundColor.a < 1f ? TextureFormat.RGBA32 : TextureFormat.RGB24, new System.Action<AsyncGPUReadbackRequest>(( asyncResult ) =>
					{
						try
						{
	...
	{
		renderersList.Clear();
		renderersList.AddRange(	target.GetComponentsInChildren<Renderer>());
