        ...
        public static void ConvertToBuildObjects()
        {
            ...
            foreach (var scene in GetAllScenes().Where(x => !x.name.EndsWith("Core")))
            {
                ...
                foreach (var element in listOfCategory)
                {
                    ...
                    var idClass = new IdClass {Id = aa, Name = element.Key.name, Path = path, Zone = element.Key.scene.name, HashCode = element.Value};
                    FindCategory(category, categories).Objects.Add(idClass);
                    aa++;
                }
                File.WriteAllText(@"BuildObjects.json", JsonConvert.SerializeObject(categories, Formatting.Indented));
